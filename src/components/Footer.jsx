    ...
    return(
        <footer ref={targetRef} className="w-full bg-white dark:bg-dark-skills">
            <div className="flex w-full justify-center items-end mx-auto pt-20 pb-50 gap-15 xs:flex-col xs:items-center lg:flex-row">
                <div className="flex justify-end text-end font-inter font-medium text-[42px] leading-[150%] text-custom-text-black dark:text-white xs:text-center lg:text-end lg:flex-none xl:flex-5 2xl:text-6xl"><span className={`footer-highlight ${isVisible ? 'active' : ''}`}>{footerSection.text} <br/> {footerSection.text_2}</span></div>
                <div className="flex items-start font-inter font-medium text-2xl leading-[150%] capitalize xs:flex-row xs:gap-9 lg:flex-none lg:gap-1 xl:flex-3 lg:flex-col 2xl:text-3xl">
